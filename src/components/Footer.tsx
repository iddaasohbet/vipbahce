"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { motion } from "framer-motion";
import { Phone, Mail, MapPin, ArrowUpRight } from "lucide-react";

interface SiteSettings {
  phone: string;
  email: string;
  address: string;
  city: string;
  district: string;
  facebook: string;
  instagram: string;
  twitter: string;
  youtube: string;
  linkedin: string;
  whatsapp: string;
  site_title: string;
  site_description: string;
  working_hours: string;
}

const defaultSettings: SiteSettings = {
  phone: "+90 533 359 34 66",
  email: "info@vipkisbahcesi.com",
  address: "İstanbul, Türkiye",
  city: "İstanbul",
  district: "",
  facebook: "https://www.facebook.com/share/1BiMn83rUP/",
  instagram: "https://www.instagram.com/vipkisbahcesi",
  twitter: "",
  youtube: "https://youtube.com/@vipkisbahcesi",
  linkedin: "",
  whatsapp: "905425263466",
  site_title: "VIP Kış Bahçesi",
  site_description: "Modern Bioklimatik Çözümler",
  working_hours: "Pazartesi - Cumartesi: 09:00 - 18:00",
};

export default function Footer() {
  const [settings, setSettings] = useState<SiteSettings>(defaultSettings);

  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const response = await fetch("/api/settings");
        const data = await response.json();
        if (data.success && data.settings) {
          setSettings(data.settings);
        }
      } catch (error) {
        console.error("Settings fetch error:", error);
      }
    };
    fetchSettings();
  }, []);

  // WhatsApp numarasını düzgün formata çevir
  const getWhatsAppNumber = () => {
    const num = settings.whatsapp || defaultSettings.whatsapp;
    // Sadece rakamları al
    return num.replace(/\D/g, '');
  };

  // Adres bilgisini oluştur
  const getFullAddress = () => {
    const parts = [];
    if (settings.address) parts.push(settings.address);
    if (settings.district) parts.push(settings.district);
    if (settings.city) parts.push(settings.city);
    return parts.length > 0 ? parts.join(', ') : 'İstanbul, Türkiye';
  };

  const links = [
    { name: 'Ana Sayfa', href: "/" },
    { name: 'Cam Tavan', href: "/cam-tavan" },
    { name: 'Sandviç Panel', href: "/sandvic-panel" },
    { name: 'Bioklimatik', href: "/bioklimatik" },
    { name: 'Rolling Roof', href: "/rolling-roof" },
    { name: 'Otomatik Pergola', href: "/otomatik-pergola" },
    { name: 'Veranda', href: "/veranda" },
    { name: 'Kış Bahçesi', href: "/kis-bahcesi" },
    { name: 'Teras Kapama', href: "/teras-kapama" },
    { name: 'Giyotin Cam', href: "/giyotin-cam" },
    { name: 'Sürgülü Cam', href: "/surgulu-cam" },
    { name: 'Katlanır Cam', href: "/katlanir-cam" },
    { name: 'Hebeshiebe', href: "/hebeshiebe" },
    { name: 'Hakkımızda', href: "/hakkimizda" },
    { name: 'Galeri', href: "/galeri" },
    { name: 'İletişim', href: "/teklif-al" },
  ];

  return (
    <footer className="relative bg-white">
      {/* Top border with gradient */}
      <div className="h-px w-full bg-gradient-to-r from-transparent via-gray-300 to-transparent" />
      <div className="mx-auto max-w-7xl px-4 md:px-6 py-12">
        {/* Top Section - Two Columns */}
        <div className="mb-12 grid gap-12 lg:grid-cols-2">
          {/* Left - Brand & Description */}
          <div>
            <Link href="/" className="group mb-6 inline-flex items-center gap-3">
              <motion.div
                whileHover={{ scale: 1.05 }}
                transition={{ duration: 0.2 }}
                className="relative flex h-11 w-11 items-center justify-center rounded-lg bg-teal-900"
                aria-hidden="true"
              >
                {/* Corporate Winter Garden Icon */}
                <svg 
                  viewBox="0 0 40 40" 
                  className="h-7 w-7"
                  fill="none" 
                  xmlns="http://www.w3.org/2000/svg"
                >
                  {/* Outer hexagon frame */}
                  <path 
                    d="M20 4L34 12V28L20 36L6 28V12L20 4Z" 
                    stroke="#ffffff"
                    strokeWidth="2"
                    fill="none"
                  />
                  {/* Inner house structure */}
                  <path 
                    d="M13 18L20 13L27 18" 
                    stroke="#ffffff"
                    strokeWidth="2" 
                    strokeLinecap="round" 
                    strokeLinejoin="round"
                  />
                  <path 
                    d="M14 18V26H26V18" 
                    stroke="#ffffff"
                    strokeWidth="2" 
                    strokeLinecap="round" 
                    strokeLinejoin="round"
                  />
                  {/* Glass panels */}
                  <line x1="18" y1="18" x2="18" y2="26" stroke="rgba(255,255,255,0.7)" strokeWidth="1.5" />
                  <line x1="22" y1="18" x2="22" y2="26" stroke="rgba(255,255,255,0.7)" strokeWidth="1.5" />
                  <line x1="14" y1="22" x2="26" y2="22" stroke="rgba(255,255,255,0.7)" strokeWidth="1.5" />
                </svg>
              </motion.div>
              <div>
                <h3 className="text-xl font-bold text-gray-900">Kış Bahçesi</h3>
                <p className="text-[10px] uppercase tracking-widest font-semibold text-gray-900">Kış bahçesi fiyatları</p>
              </div>
            </Link>
            <p className="mb-6 max-w-md text-sm leading-relaxed text-gray-600">
              Hayalinizdeki yaşam alanlarını gerçeğe dönüştürüyoruz. Modern tasarım, kaliteli malzeme ve uzman ekip ile kış bahçesi sistemleri.
            </p>
            <Link
              href="/teklif-al"
              className="group inline-flex items-center gap-2 rounded-full bg-teal-900 px-6 py-3 text-sm font-semibold text-white transition-all hover:bg-teal-800"
            >
              Ücretsiz Teklif Alın
              <ArrowUpRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
            </Link>
          </div>

          {/* Right - Links & Contact */}
          <div className="grid gap-8 sm:grid-cols-2">
            {/* Quick Links */}
            <div>
              <h4 className="mb-4 text-sm font-bold text-gray-900">Sayfalar</h4>
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                {links.map((link) => (
                  <Link
                    key={link.name}
                    href={link.href}
                    className="group flex items-center text-sm text-gray-600 transition-colors hover:text-teal-900"
                  >
                    <span className="mr-2 h-1 w-1 rounded-full bg-gray-400 transition-all group-hover:w-4 group-hover:bg-teal-900" />
                    {link.name}
                  </Link>
                ))}
              </div>
            </div>

            {/* Contact Info */}
            <div>
              <h4 className="mb-4 text-sm font-bold text-gray-900">İletişim</h4>
              <div className="space-y-4">
                <a 
                  href={`tel:${settings.phone.replace(/\s/g, '')}`}
                  className="flex items-start gap-3 text-sm text-gray-600 transition-colors hover:text-teal-900"
                >
                  <Phone className="mt-0.5 h-4 w-4 flex-shrink-0" />
                  <span>{settings.phone}</span>
                </a>
                <a 
                  href={`mailto:${settings.email}`}
                  className="flex items-start gap-3 text-sm text-gray-600 transition-colors hover:text-teal-900"
                >
                  <Mail className="mt-0.5 h-4 w-4 flex-shrink-0" />
                  <span>{settings.email}</span>
                </a>
                <div className="flex items-start gap-3 text-sm text-gray-600">
                  <MapPin className="mt-0.5 h-4 w-4 flex-shrink-0" />
                  <span>{getFullAddress()}</span>
                </div>
              </div>

              {/* Social Media Icons */}
              <div className="mt-6 flex items-center gap-3">
                {/* WhatsApp */}
                {settings.whatsapp && (
                  <a
                    href={`https://wa.me/${getWhatsAppNumber()}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-green-500 text-white transition-all hover:bg-green-600 hover:scale-110"
                    aria-label="WhatsApp"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                    </svg>
                  </a>
                )}

                {/* Instagram */}
                {settings.instagram && (
                  <a
                    href={settings.instagram}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-purple-500 via-pink-500 to-orange-400 text-white transition-all hover:scale-110"
                    aria-label="Instagram"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                    </svg>
                  </a>
                )}

                {/* YouTube */}
                {settings.youtube && (
                  <a
                    href={settings.youtube}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-red-600 text-white transition-all hover:bg-red-700 hover:scale-110"
                    aria-label="YouTube"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                    </svg>
                  </a>
                )}

                {/* Facebook */}
                {settings.facebook && (
                  <a
                    href={settings.facebook}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-600 text-white transition-all hover:bg-blue-700 hover:scale-110"
                    aria-label="Facebook"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                  </a>
                )}

                {/* Twitter */}
                {settings.twitter && (
                  <a
                    href={settings.twitter}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-black text-white transition-all hover:bg-gray-800 hover:scale-110"
                    aria-label="Twitter / X"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                    </svg>
                  </a>
                )}

                {/* LinkedIn */}
                {settings.linkedin && (
                  <a
                    href={settings.linkedin}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-700 text-white transition-all hover:bg-blue-800 hover:scale-110"
                    aria-label="LinkedIn"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                    </svg>
                  </a>
                )}
              </div>
            </div>
          </div>
        </div>

        {/* Sponsors Section */}
        <div className="mb-8 border-t border-gray-200 pt-8">
          <h4 className="mb-4 text-center text-sm font-bold text-gray-900">Sponsorlarımız</h4>
          <div className="flex flex-wrap items-center justify-center gap-6">
            <a
              href="https://www.lyyildirimlarinsaat.com/"
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-all hover:bg-teal-900 hover:text-white"
            >
              Silivri Tesisat
            </a>
            <a
              href="https://www.lyyildirimlarinsaat.com/"
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-all hover:bg-teal-900 hover:text-white"
            >
              Silivri Tadilat
            </a>
            <a
              href="https://www.lyyildirimlarinsaat.com/"
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-all hover:bg-teal-900 hover:text-white"
            >
              Silivri Duşakabin
            </a>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="flex flex-col items-center justify-between gap-4 border-t border-gray-200 pt-8 text-center md:flex-row md:text-left">
          <p className="text-xs text-gray-500">
            © {new Date().getFullYear()} Kış Bahçesi. Tüm hakları saklıdır.
          </p>
          <div className="flex items-center gap-4 text-xs text-gray-500">
            <Link href="/gizlilik" className="transition-colors hover:text-gray-900">Gizlilik</Link>
            <span>•</span>
            <Link href="/kosullar" className="transition-colors hover:text-gray-900">Koşullar</Link>
            <span>•</span>
            <Link href="/kvkk" className="transition-colors hover:text-gray-900">KVKK</Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
